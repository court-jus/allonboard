/* 
 * Options :
 * PLAYERS : an array of players for the game (if empty, the game starts on the menu)
 *
 * Available players :
 *******************************
 * Human
 *******************************
 * Runner : a simple robot that just tries to go forward
 * Picker : a simple robot that just tries to pick up cards
 * KidGardener : a strange robot that does some things
 * Weighter1 to Weighter6 : this robots are smarter
 *******************************
 * MyRobot : the custom robot you can modify in customrobot.js
 */


var PLAYERS = [];
